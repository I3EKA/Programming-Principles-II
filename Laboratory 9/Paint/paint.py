...
def Rhombus(screen, color, x, y, X1, Y1, radius):
    ...
    pygame.draw.polygon(screen, pygame.Color(
        color), (line1, line2, line3, line4), radius)
...
def RightTriangle(screen, color, x, y, X1, Y1, radius,):
    ...
    z2 = y2
    pygame.draw.polygon(screen, pygame.Color(
        color), ((x1, y1), (x2, y2), (z1, z2)), radius)
...
def EquilateralTriangle(surface, color, x, y, X1, Y1, radius):
    ...
    if y2 > y1:
        pygame.draw.polygon(surface, pygame.Color(
            color), ((x1, y1), (x2, y2), (z1, y2 - heigh)), radius)
    else:
        pygame.draw.polygon(surface, pygame.Color(
            color), ((x1, y1), (x2, y2), (z1, y1-heigh)), radius)
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                if tool == 'rectangle':
                    Rectangle(
                        screen, color, prev_pos[0], prev_pos[1], COORX[0], COORX[1], radius)
                    main_rect = True
                if tool == 'triangle':
                    EquilateralTriangle(
                        screen, color, prev_pos[0], prev_pos[1], COORX[0], COORX[1], radius)
                ...
                if tool == 'circle':
                    Circle(screen, color,
                           prev_pos[0], prev_pos[1], COORX[0], COORX[1], radius)
                    main_circle = True
                if tool == 'square':
                    Square(screen, color,
                           prev_pos[0], prev_pos[1], COORX[0], COORX[1], radius)
